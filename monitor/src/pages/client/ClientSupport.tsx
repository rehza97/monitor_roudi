...
const statusColor: Record<string, string> = {
  "Ouvert":   "text-blue-700 bg-blue-50",
  "En cours": "text-amber-700 bg-amber-50",
  "Résolu":   "text-emerald-700 bg-emerald-50",
  "Fermé":    "text-slate-600 bg-slate-100",
}
...
const INPUT_CLS =
  "w-full h-10 px-3 text-sm rounded-lg border border-slate-300 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#2463eb]"
const TEXTAREA_CLS =
  "w-full px-3 py-2 text-sm rounded-lg border border-slate-300 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#2463eb] resize-none"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="font-bold text-slate-900 text-lg">Nouveau ticket</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-1">
            <button
              ...
              className="flex-1 h-10 bg-[#2463eb] text-white font-semibold rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 h-10 border border-slate-300 text-slate-700 font-semibold rounded-lg hover:bg-slate-50 text-sm"
            >
              Annuler
            </button>
          </div>
...
export default function ClientSupport() {
  ...
  return (
    <DashboardLayout role="client" navItems={clientNav} pageTitle="Support & Maintenance">
      <div className="p-6 w-full space-y-8 bg-[#f6f6f8] min-h-[calc(100vh-64px)]">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Support et Maintenance</h2>
            <p className="text-slate-500 mt-1">Créez un ticket ou suivez l'état de vos demandes existantes.</p>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-4 py-2 bg-[#2463eb] text-white text-sm font-semibold rounded-lg hover:bg-blue-700"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-900">Mes tickets</h3>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 py-14">
              ...
              <p className="font-semibold text-slate-900">Aucun ticket ouvert</p>
              <p className="text-sm text-slate-500 text-center max-w-xs">
                Vous n'avez pas encore de ticket de support. Créez-en un si vous rencontrez un problème.
              </p>
              <button onClick={() => setShowModal(true)} className="mt-1 px-4 py-2 bg-[#2463eb] text-white text-sm font-semibold rounded-lg hover:bg-blue-700">
                Ouvrir un ticket
              </button>
            </div>
          ) : (
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-[#2463eb] to-blue-700 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
              <h3 className="font-bold text-lg mb-2 relative z-10">Besoin d'aide urgente ?</h3>
              <p className="text-blue-100 text-sm mb-4 relative z-10">Notre équipe est disponible 24/7 pour les urgences critiques.</p>
              <button className="w-full bg-white/20 hover:bg-white/30 text-white text-sm font-medium py-2 px-4 rounded-lg backdrop-blur-sm border border-white/10 transition-colors flex items-center justify-center gap-2 relative z-10">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 flex items-center gap-4 shadow-sm">
              <div className="size-12 rounded-full bg-[#2463eb] flex items-center justify-center text-white shrink-0">
                <span className="material-symbols-outlined text-[22px]">support_agent</span>
              </div>
              <div className="flex-1">
                <p className="font-semibold text-slate-900">Ligne directe</p>
