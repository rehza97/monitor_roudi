    ...
    <PublicLayout>
      <section className="bg-white py-16 border-b border-slate-200">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h1 className="text-4xl font-black tracking-tight text-slate-900 mb-4">Mentions légales</h1>
          <p className="text-slate-500">Conformément à l'article 6 de la loi n°2004-575 du 21 juin 2004</p>
        </div>
      </section>
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          {infos.map(block => (
            <div key={block.title} className="bg-white rounded-2xl border border-slate-200 p-8">
              <h2 className="text-xl font-bold text-slate-900 mb-6">{block.title}</h2>
              <dl className="space-y-3">
                {block.items.map(item => (
                  <div key={item.label} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 py-2 border-b border-slate-100 last:border-0">
                    <dt className="text-sm text-slate-500 sm:w-56 shrink-0">{item.label}</dt>
