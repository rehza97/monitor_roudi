...
export default function EngineerMessages() {
  ...
  return (
    <DashboardLayout role="engineer" navItems={engineerNav} pageTitle="Messagerie">
      <div className="flex h-[calc(100vh-64px)] bg-[#f6f6f8]">
          ...
          <div className="p-3 border-b border-slate-200">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-10 pl-9 pr-3 text-sm rounded-lg border border-slate-200 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#2463eb]/30"
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {filtered.map((t) => (
              <button
                ...
                className={`w-full text-left flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors border-b border-slate-100 ${
                  selectedId === t.id ? "bg-[#2463eb]/10 border-l-4 border-l-[#2463eb]" : ""
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 truncate">{t.last || "—"}</p>
                </div>
                {t.unread > 0 ? (
                  <span className="size-5 rounded-full bg-[#2463eb] text-white text-[10px] font-bold flex items-center justify-center shrink-0">
                    {t.unread}
                  </span>
                ) : null}
              </button>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {messages.map((m) => (
                  <div key={m.id} className={`flex items-end gap-2.5 ${m.mine ? "flex-row-reverse" : ""}`}>
                    ...
                    <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${m.mine ? "bg-[#2463eb] text-white rounded-br-sm" : "bg-slate-100 text-slate-700 border border-slate-200 rounded-bl-sm"}`}>
                      {m.text}
              ...
              <div className="px-4 py-3 bg-white border-t border-slate-200">
                <form className="flex items-center gap-2" onSubmit={(e) => void sendMessage(e)}>
                  <input
                    ...
                    className="flex-1 h-10 px-4 text-sm rounded-xl border border-slate-200 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#2463eb]/30"
                    placeholder="Écrire un message..."
                  />
                  <button
                    type="submit"
                    disabled={!input.trim()}
                    className="size-10 flex items-center justify-center rounded-xl bg-[#2463eb] hover:bg-[#1d4ed8] disabled:opacity-40 text-white transition-colors shrink-0"
                  >
