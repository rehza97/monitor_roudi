...
function eventColor(priority: string): string {
  if (priority === "Urgente") return "bg-rose-100 text-rose-700 border-rose-200"
  if (priority === "Haute") return "bg-amber-100 text-amber-700 border-amber-200"
  if (priority === "Normale") return "bg-blue-100 text-blue-700 border-blue-200"
      ...
      <div className="relative bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2.5 text-sm">
          {[
            ...
            <div key={r.icon} className="flex items-center gap-3 text-slate-600">
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setWeekOffset((w) => w - 1)} className="size-9 flex items-center justify-center rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-600 transition-colors">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="grid grid-cols-7 border-b border-slate-200">
            {days.map((d, i) => {
              ...
              return (
                <div key={d} className={`px-4 py-3 text-center border-r border-slate-200 last:border-0 ${isToday ? "bg-amber-50" : ""}`}>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{d}</p>
                  <p className={`text-xl font-bold mt-1 ${isToday ? "text-amber-600" : "text-slate-900"}`}>{date.getDate()}</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="font-bold text-slate-900 mb-4">Prochaines interventions</h3>
          {loading ? (
            ...
            <div className="space-y-2">
              {upcoming.map((ev) => (
                <button key={ev.id} onClick={() => setSelectedEvent(ev)} className="w-full flex items-center gap-4 p-3 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors text-left">
